...
void eval_cmd(char *cmd) {
    ...
    if (pid == -1) {
        ...
        if(set_limit(RLIMIT_CPU, parsedArgs->cpuSecLimit) == -1 || set_limit(RLIMIT_AS, parsedArgs->virtMemLimit) == -1) {
            printf("Cannot set limit cpu limit:%ld, virt mem limit:%ld\n", parsedArgs->cpuSecLimit, parsedArgs->cpuSecLimit);
            exit(1);
        }
...
void parse_args(int argc, char ** argv) {
    ...
    parsedArgs->cpuSecLimit = 5;
    parsedArgs->virtMemLimit = (rlim_t) 15000000;

    if (argc >= 3) {
        parsedArgs->cpuSecLimit = (rlim_t) strtol(argv[2], NULL, 10);
    }
    if (argc >= 4) {
        parsedArgs->virtMemLimit = (rlim_t) strtol(argv[3], NULL, 10);
    }
